...
export const KanbanCardDemo = React.memo(function KanbanCard(props) {
    ...
    if (!props.name || !props.blok || !props.phone ) {
        return (
            <Fragment>
                <div></div>
            </Fragment>
        ...
        return (
            <Card className={cx(styles.card)} elevation={2}>
                <div className={cx(styles.content)}>
                    <div className={cx(styles.avatar)}>
                        <Avatar src={props.image}
                                ...
                                style={{
                                    color: theme.palette.getContrastText(stringToColor(props.name)),
                                    backgroundColor: stringToColor(props.name),

                                }}
                        >{props.name.charAt(0) || 'John Doe'}</Avatar>

                    </div>
                    <div className={cx(styles.details)}>
                        <div
                            className={styles.name}>{`${props.name || 'John Doe'}`}</div>
                        <div className={styles.address}>{`${props.nickname?`${props.nickname} | `:""}Blok ${props.blok || 'F7'}`}</div>
                        <div className={cx(styles.contact)}>
                            <a className={cx(styles.contactPhone)}
                               href={`tel:${props.phone || '82155627063'}`}>{`${props.phone || '82155627063'}`}</a>
                            <Tooltip title={`Call ${props.name || 'John Doe'}`}>
                                <IconButton style={{width: '4vh', height: '4vh'}}>

                                    <a className={cx(styles.contactCall)}
                                       href={`tel:${props.phone || '82155627063'}`}><CallIcon/></a>
                                </IconButton>
                            ...
                            <Tooltip title={"Copy"}>
                                ...
                                <span className={cx(styles.contactCopy)}><FileCopyIcon onClick={() => {
                                    navigator.clipboard.writeText(`${props.phone || '82155627063'}`).then();
                                }}/></span> </IconButton>
                ...
                {/*        <h3 style={{border:"1px red solid"}} className={styles.heading}>{props.name || 'John Doe'}</h3>*/}
